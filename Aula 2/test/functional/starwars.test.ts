...
describe('Valida o comportamento de uma função', ()=>{
    ...
    it('Soma de a+b', () => {
        const funcao = new Operation()

        expect(funcao.soma(10,10)).toBe(20)
    });
    it('Subtração de a-b', () => {
        const funcao = new Operation()

        expect(funcao.subtracao(10,10)).toBe(0)
    });
    it('Multiplicação de a*b', () => {
        const funcao = new Operation()

        expect(funcao.multiplicacao(10,10)).toBe(100)
    });
});

describe('Unite testing for Starwars API', ()=> {
    it('Should return a not null list of planets', async ()=>{
        const response = await global.testRequest.get('/planets')
        expect(response).toBeNull
    })

    it('Should return a not empty test of planets', async ()=>{
        const response = await global.testRequest.get('/planets')
